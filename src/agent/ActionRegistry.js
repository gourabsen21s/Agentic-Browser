...
const types_1 = require("../shared/types"); // Import types from shared
const logger = (0, logger_1.getLogger)('ActionRegistry'); // Get a logger instance for this service
/**
 ...
 */
function convertToActionParameterSchema(paramDef) {
    let schema;
    switch (paramDef.type) {
        case 'string':
            ...
            if (paramDef.pattern)
                schema = schema.regex(new RegExp(paramDef.pattern));
            if (paramDef.enum)
                schema = schema.refine(val => paramDef.enum.includes(val), `Must be one of ${paramDef.enum.join(', ')}`);
            break;
        case 'number':
            schema = zod_1.z.number();
            // CORRECTED: Reassign the result of the method call back to schema
            if (paramDef.minimum !== undefined)
                schema = schema.min(paramDef.minimum);
            if (paramDef.maximum !== undefined)
                schema = schema.max(paramDef.maximum);
            if (paramDef.enum)
                schema = schema.refine(val => paramDef.enum.includes(val), `Must be one of ${paramDef.enum.join(', ')}`);
            break;
        case 'boolean':
        ...
        case 'array':
            if (paramDef.items) {
                schema = zod_1.z.array(convertToActionParameterSchema(paramDef.items));
            }
            else {
...
class ActionRegistry {
     ...
     */
    register(action, overwrite = false) {
     ...
     */
    validateParameters(actionName, params, deepValidate = false) {
        ...
        const fullActionSchema = zod_1.z.object(actionSchemaShape).strict();
        try {
            if (deepValidate) {
    ...
    executeAction(actionName_1) {
        return __awaiter(this, arguments, void 0, function* (actionName, params = {}) {
            const action = this.actions.get(actionName);
            if (!action) {
            ...
            try {
                const result = yield action.execute(params);
     ...
     */
    findActionByParameter(queryParams) {
