...
const logger = (0, logger_1.getLogger)('MessageManagerService');
/**
...
class MessageManager {
    constructor(task, systemMessage, settings, initialState) {
        this.task = task;
     ...
     */
    addStateMessage(browserStateSummary, result, stepInfo, useVision) {
        const contentParts = [];
        ...
        const interactiveElements = Object.values(browserStateSummary.selectorMap)
            ...
            const attrs = this.settings.include_attributes
                .map(attr => {
                var _a;
                const value = (_a = el.attributes) === null || _a === void 0 ? void 0 : _a[attr];
        ...
        this._addMessageWithTokens(new messages_1.HumanMessage({ content: contentParts }));
    }
    /**
    ...
    addPlan(plan, position = -1) {
        const planMessage = new messages_1.HumanMessage({ content: `Plan: ${plan}` });
    ...
    addModelOutput(output) {
        ...
        this._addMessageWithTokens(new messages_1.HumanMessage({ content: `Agent Action: ${actionString}` }));
    }
    /**
     ...
     */
    _addMessageWithTokens(message, position = -1) {
    ...
    addNewTask(newTask) {
        this.task = newTask;
        this._addMessageWithTokens(new messages_1.HumanMessage({ content: `New task received: ${newTask}` }));
